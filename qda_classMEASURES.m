%% Appendix 4.4: CODE TO PERFORM QDA

%%% Introduction
% This code uses
% Also use this code to create Figure 4.8
...
%% Author / Version
%
% Created by: <http://www.fredhasselman.com/site/Creations/Creations.html *Fred Hasselman*> / February 2011
%
...
cd('...');
load('observedLOGIT.mat','observedGR');
...
load('rpde_dfa3.mat','dfaSTATS','rpSTATS','rpTS');
load('hnr_noise.mat','mirSTATS');
load('formants.mat','Formants','STIM');
load('RTent.mat');
...
keep bootN observedGR DET LAM HNR NOISY IA FS TOTcl
...
%% Create Table with Classifier results to paste into wordprocessor
 ...
 for i = 1:5
  tab(i,:) = [TOTcl(i).Class(1:10) TOTcl(i).Class(11:20),...
   TOTcl(i).Class(21:30) TOTcl(i).Class(31:40)];
  tab(i+5,:) = [AVEcl(i).Class(1:10) AVEcl(i).Class(11:20),....
   AVEcl(i).Class(21:30) AVEcl(i).Class(31:40)];
  tab(i+10,:) = [DYScl(i).Class(1:10) DYScl(i).Class(11:20),...
...
for i=1:5
 for j=1:4
  cnt=cnt+1;
  subplot(5,4,cnt)
  plot(1:10,tab(i,stims(j,:)),'-k.','LineWidth',2);hold on;
  plot(1:10,observedGR(stims(j,:)',1),':ko','Color',[.5 .5 .5],'LineWidth',2); hold on;
  plot([1 10],[.5 .5],':k'); xlabel('Stimullus');
